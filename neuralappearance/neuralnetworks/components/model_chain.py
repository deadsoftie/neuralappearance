...
class ModelChain(IModel):
    """Sequential composition of two or more models.

    Python accepts any number of models, then builds nested ``ChainedModelPair``
    objects for Slang's statically typed binary composition. Initialization
    propagates each output type to the next model.
    """
...
class ChainedModelPair(IModel):
    """Represent one binary node in a Slang ``ModelChain`` type."""
    ...
    def model_data(self):
        return {'first': self.first.get_this(), 'second': self.second.get_this()}
